...
import { laterTodayISO, seedClient, seedRide, uniq } from './fixtures/mobile-seed';
...
test.describe('M3 — Driver queue on iPhone', () => {
  ...
  test('renders the driver-scoped queue with no global nav', async ({ page, request }) => {
    ...
    await seedRide(request, clientId, driverId, futureDate(30), 'MIA', 'Brickell');
  ...
  test('cards show pickup time, client name, route, and status', async ({ page, request }) => {
    ...
    await seedRide(request, clientId, driverId, futureDate(45), 'Coral Gables', 'Wynwood');
  ...
  test('rides sorted ascending by pickup time', async ({ page, request }) => {
    const driverId = `drv-${uniq()}`;
    const { id: clientId } = await seedClient(request);
    await seedRide(request, clientId, driverId, futureDate(60), 'Late', 'Spot');
    await seedRide(request, clientId, driverId, futureDate(31), 'Early', 'Spot');
    await seedRide(request, clientId, driverId, futureDate(45), 'Mid', 'Spot');
  ...
  test('today rides grouped under Today heading', async ({ page, request }) => {
    const driverId = `drv-${uniq()}`;
    const { id: clientId } = await seedClient(request);
    await seedRide(request, clientId, driverId, laterTodayISO(), 'Now', 'Here');
